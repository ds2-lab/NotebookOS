...
var _ = Describe("AllocationManager Standard Tests", func() {
	...
	Context("Static Scheduling", func() {
		...
		It("Will correctly handle the scheduling of a single pending resource request", func() {
			...
			err := resourceManager.ContainerStartedRunningOnHost(1, "Kernel1", kernel1Spec)
			Expect(err).To(BeNil())
		...
		It("Will correctly handle the scheduling of multiple pending resource request", func() {
			...
			err = resourceManager.ContainerStartedRunningOnHost(1, "Kernel2", kernel2Spec)
			Expect(err).To(BeNil())
		...
		It("Will correctly handle scheduling multiple committed resources", func() {
			...
			err = resourceManager.ContainerStartedRunningOnHost(1, "Kernel3", kernel3spec)
			...
			err = resourceManager.ContainerStartedRunningOnHost(1, "Kernel4", kernel4spec)
			Expect(err).To(BeNil())
			...
			err = resourceManager.ContainerStartedRunningOnHost(1, "Kernel5", kernel5spec)
			Expect(err).To(BeNil())
			...
			err = resourceManager.ContainerStartedRunningOnHost(1, "Kernel6", kernel6spec)
			Expect(err).To(BeNil())
			...
			err = resourceManager.ContainerStartedRunningOnHost(1, "Kernel7", kernel7spec)
			Expect(err).To(BeNil())
		...
		It("Will correctly adjust a lone pending resource reservation to a larger reservation", func() {
			...
			err := resourceManager.ContainerStartedRunningOnHost(1, "Kernel1", kernel1SpecV1)
			Expect(err).To(BeNil())
