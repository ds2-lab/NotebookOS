...
func main() {
	n := 3
	port := 19800
	peer_addresses := make([]string, n)
	peer_ids := make([]int, n)
	for i := 0; i < n; i++ {
		peer_addresses[i] = fmt.Sprintf("http://127.0.0.1:%d", port+i)
		peer_ids = append(peer_ids, i+1)
	}
	...
	for i := 0; i < n; i++ {
		nodes[i] = smr.NewLogNode(store, i+1, peer_addresses, peer_ids, false)
		if i == 0 {
