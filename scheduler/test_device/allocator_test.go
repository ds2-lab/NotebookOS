...
var _ = Describe("Allocator Tests", func() {
	...
	BeforeEach(func() {
		...
		numSpoofedPods = 4
		activePodIDs, activePods, activePodsMap := spoofPods(numSpoofedPods, 1, 4)

		mockPodCache.EXPECT().GetActivePodIDs().Return(activePodIDs).AnyTimes()
		mockPodCache.EXPECT().GetActivePods().Return(activePodsMap).AnyTimes()
		mockPodCache.EXPECT().GetPodsRunningOnNode(nodeName, string(corev1.PodPending)).Return(activePods, nil).AnyTimes()
