...
build-smr-linux-arm64:
	docker run -it --rm -v `%cd%`:/go/src/in -v `%cd%`:/out zhangjyr/gopy /bin/bash -c "cd /go/src/in/distributed_notebook && make build-smr-linux-arm64"

build-smr-linux-amd64:
	docker run -it --rm -v C:\Users\benrc\go\src\github.com\distributed-notebook:/go/src/in -v C:\Users\benrc\go\src\github.com\distributed-notebook:/out zhangjyr/gopy /bin/bash -c "cd /go/src/in/distributed_notebook && make build-smr-linux-amd64"
