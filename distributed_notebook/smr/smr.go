...
func smr_NewLogNode(storePath *C.char, id C.longlong, hdfsHostname *C.char, shouldLoadDataFromHdfs C.char, peerAddresses CGoHandle, peerIDs CGoHandle, join C.char, httpDebugPort C.longlong) CGoHandle {
	_saved_thread := C.PyEval_SaveThread()      // Release GIL
	defer C.PyEval_RestoreThread(_saved_thread) // Reacquire GIL
	return handleFromPtr_Ptr_smr_LogNode(smr.NewLogNode(C.GoString(storePath), int(id), C.GoString(hdfsHostname), boolPyToGo(shouldLoadDataFromHdfs), deptrFromHandle_Slice_string(peerAddresses), deptrFromHandle_Slice_int(peerIDs), boolPyToGo(join), int(httpDebugPort)))
