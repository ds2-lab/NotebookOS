...
#cgo CFLAGS: "-I/usr/local/include/python3.12d" -Wno-error -Wno-implicit-function-declaration -Wno-int-conversion
#cgo LDFLAGS: "-L/usr/local/lib" "-lpython3.12d" -ldl -lm
...
//export smr_LogNode_HdfsReadLatencyMilliseconds
func smr_LogNode_HdfsReadLatencyMilliseconds(_handle CGoHandle) C.longlong {
	_saved_thread := C.PyEval_SaveThread()      // Release GIL
	defer C.PyEval_RestoreThread(_saved_thread) // Reacquire GIL
	vifc, __err := gopyh.VarFromHandleTry((gopyh.CGoHandle)(_handle), "*smr.LogNode")
	if __err != nil {
	...
	return C.longlong(gopyh.Embed(vifc, reflect.TypeOf(smr.LogNode{})).(*smr.LogNode).HdfsReadLatencyMilliseconds())
...
func smr_LogNode_WriteDataDirectoryToHDFS(_handle CGoHandle, serializedState CGoHandle, resolve *C.PyObject, goRun C.char) {
	_fun_arg := resolve
	_saved_thread := C.PyEval_SaveThread()      // Release GIL
	defer C.PyEval_RestoreThread(_saved_thread) // Reacquire GIL
	vifc, __err := gopyh.VarFromHandleTry((gopyh.CGoHandle)(_handle), "*smr.LogNode")
	...
	if boolPyToGo(goRun) {
		go gopyh.Embed(vifc, reflect.TypeOf(smr.LogNode{})).(*smr.LogNode).WriteDataDirectoryToHDFS(deptrFromHandle_Slice_byte(serializedState), func(arg_0 interface{}, arg_1 string) {
			runtime.LockOSThread()
			_gstate := C.PyGILState_Ensure() // Acquire GIL
			if C.PyCallable_Check(_fun_arg) == 0 {
				C.PyGILState_Release(_gstate) // Release GIL
		...
		gopyh.Embed(vifc, reflect.TypeOf(smr.LogNode{})).(*smr.LogNode).WriteDataDirectoryToHDFS(deptrFromHandle_Slice_byte(serializedState), func(arg_0 interface{}, arg_1 string) {
			runtime.LockOSThread()
			_gstate := C.PyGILState_Ensure() // Acquire GIL
			if C.PyCallable_Check(_fun_arg) == 0 {
				C.PyGILState_Release(_gstate) // Release GIL
...
func smr_NewLogNode(storePath *C.char, id C.longlong, hdfsHostname *C.char, shouldLoadDataFromHdfs C.char, peerAddresses CGoHandle, peerIDs CGoHandle, join C.char, httpDebugPort C.longlong, deploymentMode *C.char) CGoHandle {
	_saved_thread := C.PyEval_SaveThread()      // Release GIL
	defer C.PyEval_RestoreThread(_saved_thread) // Reacquire GIL
	return handleFromPtr_Ptr_smr_LogNode(smr.NewLogNode(C.GoString(storePath), int(id), C.GoString(hdfsHostname), boolPyToGo(shouldLoadDataFromHdfs), deptrFromHandle_Slice_string(peerAddresses), deptrFromHandle_Slice_int(peerIDs), boolPyToGo(join), int(httpDebugPort), C.GoString(deploymentMode)))
