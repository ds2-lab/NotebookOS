...
from ..protocols import SyncPRID, SyncGrainWrapper
  ...
  def tell(self, profile: Optional[PickleProfile] = None) -> int:
    ...
    if profile is None:
      ...
      if profile.next_id == self.current_id:
        ...
        logging.warning("Unexpected frame: frame id {}, next id {}, current {}".format(profile.frame_id, profile.next_id, self.current_id))
    else:
...
class PickleProfiler(Pickler):
  def __init__(self, buffer: IO[bytes], pickler: Optional[Pickler]=None, *args, **kwargs):
    ...
    if hasattr(self._parent, "get_polifiller"):
      super().__setattr__("get_polifiller", self._parent.get_polifiller)
    elif hasattr(self._parent, "_get_polifiller"):
      super().__setattr__("get_polifiller", self._parent._get_polifiller)
  ...
  def mark_start_object(self, key, obj):
    ...
    profile = PickleProfile(len(self._stack), self._parent.framer.id(), next_id=self._parent.framer.sequence+1, anchor=self._parent.framer.tell())
  ...
  def _get_polifiller(self, cb: Callable[[SyncPRID], int]) -> Callable[[SyncPRID], None]:
