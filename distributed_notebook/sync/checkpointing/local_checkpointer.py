...
class LocalCheckpointer(RemoteCheckpointer):
    ...
    def read_dataset(self, pointer: DatasetPointer) -> CustomDataset:
        ...
        self.log.debug(
            f"Reading description of dataset \"{dataset_name}\" from Local Python Dict at key \"{key}\" now...")
    ...
    def __read_state_dicts(self, pointer: ModelPointer) -> tuple[
    ...
    def __write_state_dict(self, key: str, state_dict: Dict[str, Any], model_name: str = ""):
        ...
        self.log.debug(
            f"Writing state dictionary associated with model \"{model_name}\" to Local Python Dict at key \"{key}\". "
            f"Model size: {size_mb:,} MB.")

        self._data[key] = buffer.getbuffer()

        self.log.debug(
            f"Successfully wrote state of model \"{model_name}\" to Local Python Dictionary at key \"{key}\" "
            f"(model size: {size_mb} MB).")
