...
class SyncLogObject:
    ...
    def get_hash(
        ...
        if raw is None or t is int or t is float or t is bool or raw is EMPTY_TUPLE:
            ...
            pickler.persistent_id, pickle_id = self._referer.reference(
                batch, rid_provider=reference_id_provider
            )
            pickler.dump(raw)
            ...
            if self._profiling:
                ...
                pickler.print_profile(
                    lambda x: id(self._referer._dereference(x)), prmap
                )
    ...
    def _rebuild_obj_tree(
        self, raw: Any, batch: Optional[str], profiling: bool = False
    ):
