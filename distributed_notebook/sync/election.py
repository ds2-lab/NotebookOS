...
from typing import Dict, Optional, List, Type, Mapping, MutableMapping 
...
class Election(object):
    ...
    def num_proposals_received(self)->int:
        """
        The number of proposals we've received.
        """
    ...
    def num_vote_proposals_received(self)->int:
        """
        The number of 'vote' proposals we've received.
        """
    ...
    def add_vote_proposal(self, vote: LeaderElectionVote, overwrite: bool = False) -> bool:
        """
    ...
    def add_proposal(self, proposal: LeaderElectionProposal, overwrite: bool = False, received_at:float = time.time())->bool:
        """
        Register a proposed LeaderElectionProposal.
        ...
        Return True if the proposal was registered; otherwise, return False.
        The proposal may be not registered if is received after the timeout. 
        """
        ...
        if existing_proposal != None: 
            # Raise an error if we've not been instructed to overwrite. 
            if not overwrite:
                raise ValueError(f"unexpectedly received another proposal from node {proposer_id} during election term {proposal.election_term}, attempt number: {latest_attempt_number}")
            ...
            prev_attempt_number:int = existing_proposal.attempt_number
            if prev_attempt_number >= latest_attempt_number:
                raise ValueError(f"new proposal has attempt number {latest_attempt_number}, whereas previous/existing proposal has attempt number {prev_attempt_number}")
